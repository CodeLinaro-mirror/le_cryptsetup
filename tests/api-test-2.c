			    ...
			    uint64_t *r_payload_offset)
{
	...
	if (!default_header_size) {
		if (crypt_init(&c_d, THE_LOOP_DEV))
			return -EINVAL;
		if (crypt_format(c_d, CRYPT_LUKS2, "aes", "xts-plain64", NULL, NULL, 64, NULL)) {
			crypt_free(c_d);
			return -EINVAL;
		}

		default_header_size = crypt_get_data_offset(c_d);
...
static int set_fast_pbkdf(struct crypt_device *c_d)
{
	...
	return crypt_set_pbkdf_type(c_d, pbkdf);
}
...
static key_serial_t _kernel_key_by_segment(struct crypt_device *c_d, int segment)
{
	char key_description[1024];

	if (snprintf(key_description, sizeof(key_description), "cryptsetup:%s-d%u", crypt_get_uuid(c_d), segment) < 1)
		return -1;

	return request_key("logon", key_description, NULL, 0);
}

static int _volume_key_in_keyring(struct crypt_device *c_d, int segment)
{
	return _kernel_key_by_segment(c_d, segment) >= 0 ? 0 : -1;
}

static int _drop_keyring_key(struct crypt_device *c_d, int segment)
{
	key_serial_t kid = _kernel_key_by_segment(c_d, segment);
...
static int test_open(struct crypt_device *c_d __attribute__((unused)),
...
static int test_validate(struct crypt_device *c_d __attribute__((unused)), const char *json)
{
...
static int _crypt_load_check(struct crypt_device *c_d)
{
#ifdef HAVE_BLKID
	return crypt_load(c_d, CRYPT_LUKS, NULL);
#else
	return -ENOTSUP;
#endif
}
